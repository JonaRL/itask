try:
  ...
  error = Tk()
  error.title("Ooops!")
  Label(error, font='Helvetica 11 bold', pady=4, padx=4, text="Something went wrong!").grid(row=0, column=0)
  tw = Text(error, font='Helvetica 11', pady=4, padx=4)
  tw.insert("0.0", str(tb))
  tw['state'] = "disabled"
  tw.grid(row=1, column=0)
  Label(error, font='Helvetica 11', text="Bitte melde diesen Fehler unter https://github.com/jonarl/itask,").grid(row=2, column=0)
  Label(error, font='Helvetica 11', text="damit er behoben werden kann. Vielen Dank!").grid(row=3, column=0)
  mainloop()